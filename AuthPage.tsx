...
const AuthPage: React.FC = () => {
  ...
  // Custom Math reCAPTCHA state
  const [mathChallenge, setMathChallenge] = useState<MathChallenge>({ question: '', answer: 0 });
  const [userAnswer, setUserAnswer] = useState('');

  const { signup, attemptLogin, continueWithGoogle, completeGoogleSignup, resetPassword, confirmPasswordResetWithCode, emailSystemType } = useContext(UserContext);
  ...
  useEffect(() => {
    generateChallenge();
  }, [isLoginView, isSetupView, isForgotPassView]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSetupView) {
      ...
      if (parseInt(userAnswer) !== mathChallenge.answer) {
          setError('Incorrect reCAPTCHA answer. Please try again.');
          generateChallenge();
          setIsLoading(false);
          return;
      }
      const result = await attemptLogin(email, password, 'CUSTOM_MATH_RECAPTCHA');
      if (result !== 'SUCCESS') {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-brand-gray-900 to-brand-gray-950 p-4">
      <div className="w-full max-w-md p-8 space-y-6 bg-brand-gray-900 rounded-2xl shadow-2xl shadow-brand-blue/20 border border-brand-gray-800">
        ...
        {!isForgotPassView && (
            <>
                <button 
                    ...
                    type="button"
                    disabled={isLoading}
                    className="w-full flex justify-center items-center gap-3 py-3 font-semibold text-brand-gray-900 bg-white rounded-lg hover:bg-gray-100 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed mb-4"
                >
                ...
                <div className="relative flex py-2 items-center">
                    <div className="flex-grow border-t border-brand-gray-700"></div>
                    <span className="flex-shrink-0 mx-4 text-brand-gray-500 text-xs">OR CONTINUE WITH EMAIL</span>
                    <div className="flex-grow border-t border-brand-gray-700"></div>
                </div>
            </>
        )}
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {!isForgotPassView && (
            <>
                <div className="p-4 bg-brand-gray-800 border border-brand-gray-700 rounded-lg flex items-center justify-between">
                    <div className="flex flex-col">
                        <span className="text-xs font-black uppercase tracking-widest text-brand-gray-500">Security Check</span>
                        <span className="text-lg font-bold text-white">{mathChallenge.question}</span>
                    </div>
                    <input 
                        type="number" 
                        placeholder="Answer" 
                        ...
                        className="w-24 py-2 px-3 text-center text-white bg-brand-gray-900 border border-brand-gray-700 rounded-lg focus:ring-2 focus:ring-brand-blue focus:outline-none"
                        required
                    />
                </div>
