...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const publicCol = collection(db, 'users_public');
    const unsubscribe = onSnapshot(publicCol, (snapshot) => {
      setTotalUsers(snapshot.size);
      
      let totalRium = 0;
      snapshot.forEach((doc) => {
        const data = doc.data();
        totalRium += (data.riumBalance || 0);
      });
      setCirculatingSupply(totalRium);
    }, (error) => {
      console.error("Error counting users and calculating supply from public profile:", error);
    });
    return () => unsubscribe();
  }, []);
  ...
  // Global Migration: tsrBalance -> riumBalance & Sync Public Profiles
  useEffect(() => {
    const migrateAndSync = async () => {
      try {
        const migrationRef = doc(db, 'system_settings', 'migration_v3_sync_public');
        const migrationSnap = await getDoc(migrationRef);
        if (migrationSnap.exists() && migrationSnap.data().completed) return;

        console.log("Starting global migration & public profile sync...");
        ...
        for (const userDoc of querySnapshot.docs) {
          ...
          if (data.tsrBalance !== undefined) {
            updates.riumBalance = data.tsrBalance;
            updates.tsrBalance = deleteField();
          }
          ...
          const publicDocRef = doc(db, 'users_public', data.id);
          await setDoc(publicDocRef, {
            id: data.id,
            username: data.username,
            avatar: data.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${data.username}`,
            riumBalance: updates.riumBalance ?? data.riumBalance ?? 0,
            totalTrustEarned: data.totalTrustEarned || 0,
            referrals: data.referrals || 0,
            updatedAt: new Date().toISOString()
          }, { merge: true });
        }
        await setDoc(migrationRef, { completed: true, date: new Date().toISOString() });
        console.log("Global migration & sync completed.");
      } catch (error) {
        console.error("Global migration/sync failed (likely not an admin):", error);
      }
    };
    migrateAndSync();
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userDocRef, async (docSnap) => {
      if (docSnap.exists()) {
          ...
          // Sync public profile on every change
  ...
  const applyReferralLogic = async (referralCode: string, newUser: User) => {
      try {
        const referrerQuery = query(collection(db, 'users'), where('username', '==', referralCode.toLowerCase()));
        ...
        if (!referrerSnap.empty) {
          ...
          const newReferralForReferrer: Referral = {
            ...
            username: newUser.username,
            avatar: `https://i.pravatar.cc/40?u=${newUser.username}`,
            kycStatus: KycStatus.NotSubmitted,
          ...
          // Sync referrer's public profile
          await setDoc(doc(db, 'users_public', referrerDoc.id), {
            referrals: newReferralsCount,
            updatedAt: new Date().toISOString()
          }, { merge: true });
        }
      } catch (referralError) {
          console.error("Failed to apply referral bonus:", referralError);
      }
  }
